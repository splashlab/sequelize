  ...
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    if (this._dialect.supports.inserts.updateOnDuplicate && options.updateOnDuplicate) {
      if (this._dialect.supports.inserts.updateOnDuplicate == ' ON CONFLICT DO UPDATE SET') { // postgres
        // If no conflict target columns were specified, use the primary key names from options.primaryKeys
        onDuplicateKeyUpdate = ` ON CONFLICT (${options.primaryKeys.join(',')}) DO UPDATE SET ${options.updateOnDuplicate.map(attr => {
